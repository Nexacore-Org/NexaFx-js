...
export class UsersService {
  ...
  async updateProfile(
    ...
    try {
      ...
      auditDto.action = 'UPDATE_PROFILE';
      auditDto.entity = 'User';
      auditDto.entityId = userId;
      auditDto.beforeSnapshot = before;
      auditDto.afterSnapshot = {
        firstName: saved.firstName,
        lastName: saved.lastName,
      };
      auditDto.description = `User ${userId} updated their profile`;
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { passwordHash, ...profile } = saved as UserEntity & {
      passwordHash?: string;
    };
    return profile;
  }
