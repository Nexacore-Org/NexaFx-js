import {
  ...
  export class SecretsService {
    ...
    async create(createSecretDto: CreateSecretDto): Promise<SecretResponseDto> {
      ...
      this.logger.log(`Secret created: ${savedSecret.name} (ID: ${savedSecret.id})`);
    ...
    async update(id: string, updateSecretDto: UpdateSecretDto): Promise<SecretResponseDto> {
      ...
      this.logger.log(`Secret updated: ${updatedSecret.name} (ID: ${updatedSecret.id})`);
    ...
    async rotate(id: string, rotateSecretDto: RotateSecretDto): Promise<SecretResponseDto> {
      const secret = await this.findSecretById(id);
      ...
      try {
        ...
        const newValue = rotateSecretDto.newValue || this.generateSecretValue(secret.type);
        ...
        this.logger.log(`Secret rotated: ${updatedSecret.name} (ID: ${updatedSecret.id})`);
        ...
        this.logger.error(`Failed to rotate secret ${secret.name}: ${error.message}`);
    ...
    async bulkRotate(secretIds: string[], notifyServices = true): Promise<SecretResponseDto[]> {
    ...
    private async notifyAffectedServices(secret: Secret, newValue: string): Promise<void> {
      if (!secret.affectedServices || secret.affectedServices.length === 0) {
        return;
      }
  
      const notificationPromises = secret.affectedServices.map(async (service) => {
        try {
          ...
          this.logger.log(`Notified service ${service.name} of secret rotation`);
        } catch (error) {
          this.logger.error(`Failed to notify service ${service.name}: ${error.message}`);
        }
      });
    ...
    private toResponseDto(secret: Secret): SecretResponseDto {
      return {
        ...
        affectedServices: secret.affectedServices?.map(service => ({
          id: service.id,
          name: service.name,
